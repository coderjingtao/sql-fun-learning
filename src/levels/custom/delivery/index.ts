...
export default {
  ...
  answer:
    ...
    "        ROUND(SUM(CASE WHEN delivery_status = 'Delivered' THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 1) || '%' AS success_rate,\n" +
    ...
    "        RANK() OVER (ORDER BY SUM(CASE WHEN delivery_status = 'Delivered' THEN 1 ELSE 0 END) * 100.0 / COUNT(*) DESC) AS success_rank\n" +
    "    FROM \n" +
    "        delivery_records\n" +
    "    WHERE \n" +
  ...
  hint: "Use JULIANDAY to calculate time difference, combine window functions and subqueries, handle percentage string in sorting",
  type: "custom",
  difficulty: 2,
} as LevelType;
